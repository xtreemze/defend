...
`showcase/manifest.mjs` is the shared source of feature names, descriptions, paths, filenames, viewport intent, 60 fps capture targets, and publication labels. `showcase/capture.mjs` uses Playwright-controlled application time plus `captureStream(0)`/`requestFrame()` to record exactly 180 real canvas samples for every three-second scene, with VP8 preferred over VP9; `showcase/verify.mjs --source-only` rejects any dropped source frame before encoding. `showcase/render.mjs` retimes those source frames to 60 fps and creates H.264 scene videos/reels and 60 fps animated WebPs; the final verifier checks decoded H.264 cadence and parses WebP animation-frame chunks directly.
